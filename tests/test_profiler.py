...
from profiler import (
    missing_value_matrix, dtype_consistency_check, cardinality_analysis,
    correlation_matrix, build_profiling_report,
)
...
def test_dtype_consistency_flags_mixed_column():
    ...
    assert "a" in dtype_consistency_check(df)


def test_cardinality_analysis_flags_constant_column():
    df = pd.DataFrame({"a": [1, 1, 1, 1]})
    assert cardinality_analysis(df)["a"]["is_likely_constant"] is True


def test_cardinality_analysis_flags_id_like_column():
    df = pd.DataFrame({"id": [1, 2, 3, 4]})
    assert cardinality_analysis(df)["id"]["is_likely_id_like"] is True


def test_correlation_matrix_empty_for_single_numeric_col():
...
def test_build_profiling_report_has_all_sections(tmp_path):
    ...
    for key in ["metadata", "missing_value_matrix", "dtype_consistency_issues",
